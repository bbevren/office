...
// Helper functions to reduce repetitive response code
...
function sendFile(res, data, contentType, filename) {
    const headers = { 'Content-Type': contentType, 'Content-Length': data.length, ...CORS_HEADERS };
...
const server = http.createServer((req, res) => {
    ...
    // Log requests (skip static files and polling)
    if (!pathname.endsWith('.js') && !pathname.endsWith('.css') && !pathname.endsWith('.png') && !pathname.endsWith('.woff2') && pathname !== '/api/latest-download') {
    ...
    // Relaxed check to catch any downloadas request
    if ((req.method === 'POST' || req.method === 'GET') && pathname.includes('/downloadas/')) {
    ...
    // Handle PDF conversion from document binary (POST /api/convert-to-pdf)
    // This is for direct PDF download without needing pdfbin from OnlyOffice
    if (req.method === 'POST' && pathname === '/api/convert-to-pdf') {
        handleConvertToPdf(req, res);
        return;
    }
    ...
    if (req.method === 'GET' && pathname === '/api/latest-download') {
        ...
        if (latest && Date.now() - latest.timestamp < 30000) {
            ...
            sendJson(res, 200, { ready: true, url: latest.url, title: latest.title, id: latest.id });
        } else {
    ...
    if (imageMatch && req.method === 'GET') {
        ...
        if (cached) {
            ...
            res.writeHead(200, { 'Content-Type': cached.mimeType, 'Cache-Control': 'public, max-age=3600', ...CORS_HEADERS });
            res.end(cached.data);
            return;
        }
    }
    ...
    // Handle formula-lang resources (redirect to spreadsheet editor resources)
    else if (pathname.startsWith('/resources/formula-lang/')) {
        filePath = path.join(__dirname, 'public', 'onlyoffice', 'v8', 'web-apps', 'apps', 'spreadsheeteditor', 'main', 'resources', 'formula-lang', pathname.replace('/resources/formula-lang/', ''));
    }
    ...
    fs.readFile(filePath, (err, content) => {
        if (err) {
            ...
            const msg = err.code === 'ENOENT' ? 'File not found: ' + pathname : 'Server error: ' + err.code;
            console.log(`❌ ${code}: ${pathname} -> ${filePath}`);
            res.writeHead(code);
            res.end(msg);
        } else {
    ...
    const processRequest = async () => {
        try {
            ...
            if (!x2tReady) {
                ...
                sendJson(res, 503, { error: 1, message: 'Converter not ready' });
                return;
            }
            
            // For GET requests with no body, return response telling OnlyOffice to use its local document
            if (body.length === 0 && req.method === 'GET') {
                console.log('[Download] GET request with no body - returning empty document response');
                sendJson(res, 200, { error: 0, key: cmd.id || 'doc_unknown', url: '', end: true });
                return;
            }
            ...
            // Ensure the filename extension matches the actual output format
            // This prevents issues where a stale X-Override-Filename (e.g. .docx) is applied to a PDF download
            const currentExt = path.extname(title).toLowerCase().replace('.', '');
            if (currentExt !== outputExt) {
                 console.log(`[Download] Fixing extension mismatch: ${title} -> .${outputExt}`);
                 if (currentExt) {
                     // Replace existing extension
                     title = title.substring(0, title.lastIndexOf('.')) + '.' + outputExt;
                 } else {
                     // Append extension
                     title = title + '.' + outputExt;
                 }
            }
...
<TaskQueueDataConvert xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <m_nCsvDelimiter>4</m_nCsvDelimiter>
                ...
                global.downloadCache[downloadId] = { data: outputData, title, expires: Date.now() + 300000 };
                console.log('[Download] Stored in cache with ID:', downloadId, 'Title:', title);
                console.log('[Download] Cache keys:', Object.keys(global.downloadCache));
                
                // Also store for client polling
                const downloadUrl = `http://localhost:${PORT}${downloadPath}`;
                global.latestDownload = { id: downloadId, url: downloadUrl, title, timestamp: Date.now() };
                
                console.log('[Download] Sending response with URL:', downloadUrl);
                sendJson(res, 200, { error: 0, key: docId, url: downloadUrl, urls: downloadUrl, fileType: outputExt, end: true });
                
            } catch (convErr) {
                console.error('[Download] Conversion error:', convErr);
                sendJson(res, 200, { error: 1, message: convErr.message });
            }
...
function handleOpenConvert(req, res) {
    ...
    req.on('end', async () => {
        try {
            ...
            if (!x2tReady) {
                return sendError(res, 503, 'Converter not ready');
            }
...
<TaskQueueDataConvert xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <m_nCsvDelimiter>4</m_nCsvDelimiter>
            ...
            try {
                ...
                if (x2t.FS.analyzePath(mediaPath).exists) {
                    ...
                    for (const file of files) {
                        ...
                        imageDataUrls[file] = `data:${mimeType};base64,${base64}`;
                        global.imageCache[file] = { data: Buffer.from(imageData), mimeType };
            ...
            sendJson(res, 200, { success: true, data: outputData, size: outputData.length, format: 'cryptpad', images: imageDataUrls });
    ...
    req.on('end', async () => {
        try {
            ...
            const ext = contentType.includes('jpeg') || contentType.includes('jpg') ? '.jpg' :
                       contentType.includes('gif') ? '.gif' : contentType.includes('bmp') ? '.bmp' : '.png';
            
            const imageCount = Object.keys(global.imageCache).length + 1;
            const filename = `image${imageCount}${ext}`;
            global.imageCache[filename] = { data: body, mimeType: contentType };
            
            console.log('[ImageUpload] Cached image:', filename, body.length, 'bytes');
            sendJson(res, 200, { success: true, filename, url: `/onlyoffice/v8/web-apps/apps/documenteditor/main/${filename}` });
        } catch (err) {
...
function handleFetchImage(req, res, urlObj) {
    ...
    if (!imageUrl) return sendError(res, 400, 'Missing url parameter');
    ...
    protocol.get(imageUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (response) => {
        if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
            ...
            redirectProtocol.get(redirectUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } }, handleResponse).on('error', handleError);
            return;
        }
    ...
    function handleResponse(response) {
        if (response.statusCode !== 200) return sendError(res, response.statusCode, 'Failed to fetch image: ' + response.statusCode);
        ...
        response.on('end', () => {
            ...
            sendJson(res, 200, { success: true, dataUrl, size: buffer.length });
        });
    }
...
function handleFileDownload(req, res, pathname) {
    console.log('[FileDownload] Request:', pathname);
    const parts = pathname.split('/').filter(p => p);
    if (parts.length < 2) { 
        console.log('[FileDownload] Invalid path parts:', parts);
        sendError(res, 404, 'Not found'); 
        return; 
    }
    
    const downloadId = parts[1];
    console.log('[FileDownload] Looking for ID:', downloadId);
    
    const cache = global.downloadCache || {};
    console.log('[FileDownload] Available IDs:', Object.keys(cache));
    
    const item = cache[downloadId];
    
    if (!item) {
        console.log('[FileDownload] ID not found in cache');
        sendError(res, 404, 'Download not found'); 
        return;
    }
    
    if (Date.now() > item.expires) {
        console.log('[FileDownload] Download expired');
        delete cache[downloadId];
        sendError(res, 404, 'Download expired'); 
        return;
    }
    ...
    sendFile(res, item.data, contentType, item.title);
    delete cache[downloadId];
}
...
async function handleDirectConvert(req, res) {
    ...
    req.on('end', async () => {
        try {
            ...
            if (!boundaryMatch) { sendError(res, 400, 'Missing multipart boundary'); return; }
            ...
            if (!fileData) { sendError(res, 400, 'Missing file data'); return; }
            if (!x2tReady) { sendError(res, 503, 'x2t converter not ready'); return; }
            
            console.log('[DirectConvert] Converting to:', outputFormat, 'size:', fileData.length);
            
            try {
                initWorkDir();
                const inputPath = `/working/input.bin`;
                const outputPath = `/working/output.${outputFormat}`;
                
                x2t.FS.writeFile(inputPath, fileData);
                x2t.FS.writeFile('/working/params.xml', `<?xml version="1.0" encoding="utf-8"?>
<TaskQueueDataConvert xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <m_sFontDir>/working/fonts/</m_sFontDir>
  <m_sThemeDir>/working/themes</m_sThemeDir>
  <m_sFileFrom>${inputPath}</m_sFileFrom>
  ...
  <m_bIsNoBase64>true</m_bIsNoBase64>
                ...
                sendFile(res, Buffer.from(outputData), MIME_TYPES[ext] || 'application/octet-stream', title);
                
            } catch (convErr) {
                console.log('[DirectConvert] Conversion failed:', convErr.message);
                sendError(res, 500, 'Conversion failed: ' + convErr.message);
            }
            
        } catch (err) {
            console.error('[DirectConvert] Error:', err);
            sendError(res, 500, 'Conversion error: ' + err.message);
        }
    });
}

// Handle direct PDF conversion from document binary
// This converts internal document format (.bin) directly to PDF
async function handleConvertToPdf(req, res) {
    console.log('[ConvertToPdf] Handler invoked');
    ...
    req.on('end', async () => {
        try {
            const body = Buffer.concat(chunks);
            console.log('[ConvertToPdf] Received', body.length, 'bytes');
            ...
            console.log('[ConvertToPdf] Parsed parts:', Object.keys(parts));
            
            const docBin = parts.docbin;
            const filename = parts.filename || 'document';
            
            if (!docBin) {
                throw new Error('Missing docbin');
            }
            
            console.log('[ConvertToPdf] docbin:', docBin.length, 'bytes');
            ...
            // Write document to WASM filesystem
            x2t.FS.writeFile('/working/input.bin', docBin);
            
            console.log('[ConvertToPdf] Written docbin to WASM FS');
...
<TaskQueueDataConvert xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <m_bIsNoBase64>false</m_bIsNoBase64>
            ...
            console.log('[ConvertToPdf] Running x2t conversion...');
            const result = x2t.ccall("main1", "number", ["string"], ["/working/params.xml"]);
            
            console.log('[ConvertToPdf] x2t result:', result);
            ...
            console.log('[ConvertToPdf] Generated PDF:', pdfData.length, 'bytes');
            
            // Send PDF back
            sendFile(res, Buffer.from(pdfData), 'application/pdf', filename + '.pdf');
    ...
    req.on('end', async () => {
        try {
            ...
            if (pdfHeader === '%PDF') {
                ...
                sendFile(res, pdfBin, 'application/pdf', filename + '.pdf');
                return;
            }
